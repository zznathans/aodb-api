...
def test_search_with_no_matches_returns_200_not_error(client):
    ...
    resp = client.get("/", params={"output": "aoml", "search": "Nonexistent Item"})
...
def test_ql_filter(client):
    ...
    resp = client.get("/", params={"output": "aoml", "search": "Notum", "ql": 200})
...
def test_icons_false_omits_icon_tag(client):
    ...
    resp = client.get("/", params={"output": "aoml", "search": "Notum", "icons": "false"})

    assert "<img src=rdb://" not in resp.text


def test_unsupported_output_format_still_returns_200_body(client):
    resp = client.get("/", params={"output": "json", "search": "x"})
...
def test_empty_store_still_returns_200(client):
    resp = client.get("/", params={"output": "aoml", "search": "anything"})
